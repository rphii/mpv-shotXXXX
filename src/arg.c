...
static const Specify static_specify[ARG__COUNT] = {
    ...
    [ARG_INPUT] = SPECIFY(SPECIFY_STRING),
};
...
ErrDeclStatic arg_static_execute(Arg *arg, ArgList id, Str *argY)
{
    ...
    switch(id) {
        ...
        case ARG_OUTPUT: { to_verify = &arg->parsed.output; } break;
        case ARG_INPUT: { to_verify = &arg->parsed.input; } break;
...
ErrDeclStatic static_arg_parse_spec(Arg *args, ArgList arg, Str *argY, Specify spec, bool *argY_consumed)
{
    ...
    void *to_set = 0;
    switch(arg) {
        case ARG_OUTPUT: { to_set = &args->parsed.output; } break;
        case ARG_INPUT: { to_set = &args->parsed.input; } break;
...
int arg_parse(Arg *args, size_t argc, const char **argv) /* {{{ */
{
    ...
    str_trim(&args->parsed.output);
    if(!str_length(&args->parsed.input)) {
        THROW("expecting " F("%s", BOLD), static_arg[ARG_INPUT][1]);
    }
    if(!str_length(&args->parsed.output)) {
        THROW("expecting " F("%s", BOLD), static_arg[ARG_OUTPUT][1]);
    }
#endif
    return 0;
error:
...
void arg_free(Arg *arg)
{
    ...
    str_free(&arg->parsed.input);
    str_free(&arg->parsed.output);
